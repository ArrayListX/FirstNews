...
import com.tt.common.util.IPUtils;
...
public class NewsLogAspect {
	...
	@Pointcut("@annotation(com.tt.common.annotation.RequiredLog)")
	...
	private void saveLog(ProceedingJoinPoint pj, long start) throws Exception {
		...
		NewsAdmin user=(NewsAdmin) SecurityUtils.getSubject().getPrincipal();
		//2、封装用户行为日志(SysLog)
		NewsLog entity=new NewsLog();
		entity.setIp(IPUtils.getIpAddr());//IP地址
		entity.setUsername(user.getUsername());//用户名:登录用户
